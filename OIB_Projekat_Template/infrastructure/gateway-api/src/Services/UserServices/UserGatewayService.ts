...
import { UserDTO } from "../../Domain/DTOs/AuthDTOs/UserDTO";
...
export class UserGatewayService implements IUserGatewayService {
  ...
  async getUserById(id: number): Promise<UserDTO> { 
    try {
      console.log("UserGatewayService: Fetching user", id, "from User service");
      const response = await this.userClient.get<UserDTO>(`/users/${id}`);
      console.log("UserGatewayService: User found:", response.data.username);
      return response.data;
    } catch (error) {
      console.error("UserGatewayService: Get user by ID error.");
  ...
  async createUser(data: CreateUserDTO, token?: string): Promise<UserDTO> {
    try {
      console.log("UserGatewayService: Creating new user");
      const response = await this.userClient.post<UserDTO>("/users", data, {
        headers: token ? { Authorization: `Bearer ${token}` } : undefined,
      });
      console.log("UserGatewayService: User created:", response.data.username);
      return response.data;
    } catch (error) {
      console.error("UserGatewayService: Create user error.");
